...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo admin_url();?>doctors/indexAjax/" + pageindex + "?keywords=" + keywords, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo admin_url();?>doctors/edit/" + id + ".html?pageindex=" + pageindex + "&keywords=" + keywords;	
	}
	...
	function del_it(id){
		layer.confirm('删除管理员后，数据不可以恢复，您确定要这么做吗', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>doctors/del", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	function changes_do(act,id)
	{
		if(form_status==0 || form_status==3)
		{
			$.ajax({url:"<?php echo admin_url();?>doctors/changes", 
			type: 'POST', 
			data:{id:id,act:act},
			dataType: 'html', 
			timeout: 15000, 
				...
				success:function(result){
					...
					if(result.indexOf("|")>0){
						...
						if(arr[0]==100){
							...
							if(act==1)
							{
								$('#loadeds_' + id).html('<a href="javascript:changes_do(2,' + id + ');" style="color:green;font-weight:blod;">√</a>');
							}
							else
							{
								$('#loadeds_' + id).html('<a href="javascript:changes_do(1,' + id + ');" style="color:red;font-weight:blod;">×</a>');
							}
						}else if(arr[0]==200){
							layer.alert('登录状态已失效，请重新登录！', {
								icon: 2,
...
<div class="page-container">
	<div class="text-c"> 
    <input type="text" class="input-text" style="width:250px" placeholder="医生姓名" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
