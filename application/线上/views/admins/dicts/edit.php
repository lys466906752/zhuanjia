...
<article class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tr class="text-c" id="show_a">
			  ...
			  <td align="left" valign="middle"><input type="text" class="input-text" value="<?php echo $res['desc'];?>" placeholder="请填写字典名称" id="desc" name="desc"></td>
		  </tr>
			<tr class="text-c" id="show_a">
			  <td><font color='#cc0000'>*</font> 字段名称：</td>
			  <td align="left" valign="middle">
			  	<input type="text" class="input-text" value="<?php echo $res['field'];?>" placeholder="请填写字段名称，必须为英文" id="fields" name="fields">
			...
			<tr class="text-c" id="show_a">
			  ...
			  <td align="left" valign="middle">
			  	<select id="model" name="model" class="input-text">
			  		<option value="1" <?php if($res['model']==1){?>selected<?php }?>>------文本域类型------</option>
			  		<option value="2" <?php if($res['model']==2){?>selected<?php }?>>------图片类型------</option>
			  		<option value="3" <?php if($res['model']==3){?>selected<?php }?>>------编辑器类型------</option>
			  	</select>

			  </td>
		  </tr>
			<tr class="text-c">
			  <td><font color='#cc0000'>*</font> 排序编号：</td>
			  <td align="left" valign="middle">
			  	<input type="text" class="input-text" value="<?php echo $res['sort'];?>" placeholder="仅允许为整数，数字越小，排名越靠前" id="sort" name="sort">
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			else if(sorts=='' || isNaN(sorts))
			{
				layer.msg('请填写正确的排序编号');	
			}
			else
			{
				layer.closeAll();
				...
				$.ajax({url:"<?php echo admin_url();?>dicts/updates/<?php echo $res['id'];?>", 
				type: 'POST', 
				data:{desc:desc,field:fields,model:model,sorts:sorts}, 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
						layer.closeAll();
